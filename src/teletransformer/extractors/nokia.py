...
def extract_text(provider_path, input_files, working_path):
    ...
    try:
        columns_to_keep = NOKIA_TEXT_COLUMNS_TO_READ
        ...
        extracted_ddf.columns = extracted_ddf.columns.map(NOKIA_TEXT_COLUMNS_TO_READ)

        extracted_ddf['QT_TEMPO_CONVERSACAO'] = extracted_ddf['QT_TEMPO_CONVERSACAO'].fillna("0").astype(int)

        extracted_ddf["DT_CHAMADA"] = dd.to_datetime(
            extracted_ddf["DT_CHAMADA"], errors="coerce"
        )
        extracted_ddf["HORA_CHAMADA"] = extracted_ddf["DT_CHAMADA"].dt.time
        extracted_ddf["DT_CHAMADA"] = extracted_ddf["DT_CHAMADA"].dt.date

        extracted_ddf["CO_CGI_CELULA"] = (
            "724-"
            + extracted_ddf["TecnologiaCelula-MCC-MNC"]
            + "-"
            + extracted_ddf["LAC"]
            + "-"
            + extracted_ddf["1stCellA"]
        )

        extracted_ddf = extracted_ddf[NOKIA_RECORD_COLUMNS]
